                    ...
                    if (el.insertAdjacentHTML) {
                        ...
                        if (hash[where]) {
                            ...
                            if (range.createContextualFragment) {
                                ...
                                (frag = document.createDocumentFragment()), (temp = document.createElement('div'));
    ...
    onNodeOver: function (n, dd, e, data) {
        var D = Ext.ensible.Date,
            start = data.type == 'eventdrag' ? n.date : D.min(data.start, n.date),
                ...
                data.type == 'eventdrag'
                    ? n.date.add(Date.DAY, D.diffDays(data.eventStart, data.eventEnd))
            ...
            if (data.type == 'eventdrag') {
                var rec = this.view.getEventRecordFromEl(data.ddel),
                    dt = Ext.ensible.Date.copyTime(
                        rec.data[Ext.ensible.cal.EventMappings.StartDate.name],
                        n.date,
                    );
    ...
    onNodeOver: function (n, dd, e, data) {
        ...
        if (data.type == 'caldrag') {
            ...
            dt = String.format(
                ...
                this.dragStartDate.format(timeFormat),
                this.dragEndDate.format(timeFormat),
            );
        } else {
...
Ext.ensible.cal.WeekEventRenderer = (function () {
    ...
    var renderEvent = function (
        ...
        var eventMappings = Ext.ensible.cal.EventMappings,
            ...
            daysToEventEnd =
                Ext.ensible.Date.diffDays(currentDate, eventData[eventMappings.EndDate.name]) + 1,
        ...
        eventData.spanCls = eventData.spanLeft
            ? eventData.spanRight
                ? 'ext-cal-ev-spanboth'
                : 'ext-cal-ev-spanleft'
            : eventData.spanRight
...
Ext.ensible.cal.RecurrenceField = Ext.extend(Ext.form.Field, {
    ...
    initSubComponents: function () {
        ...
        this.weekly = new Ext.ensible.cal.recurrenceBase({
            ...
            getValue: function () {
                var v = '',
                    sel = Ext.getCmp(this.id + '-days').getValue();
                    ...
                    function (p) {
                        if (p.indexOf('COUNT') > -1) {
                            ...
                            Ext.getCmp(this.id + '-date')
                                .setValue(Date.parseDate(dt, this.untilDateFormat))
                                .show();
...
Ext.ensible.cal.DateRangeField = Ext.extend(Ext.form.Field, {
    ...
    onRender: function (ct, position) {
        if (!this.el) {
            ...
            if (singleLine == 'auto') {
                var el,
                    w = this.ownerCt.getWidth() - this.ownerCt.getEl().getPadding('lr');
    ...
    setValue: function (v) {
        ...
        } else if (v[Ext.ensible.cal.EventMappings.StartDate.name]) {
            //object
...
Ext.ensible.cal.CalendarView = Ext.extend(Ext.BoxComponent, {
            ...
            for (d = 0; d < this.dayCount; d++) {
                if (evtsInView.getCount() > 0) {
                    var evts = evtsInView.filterBy(function (rec) {
                        var startsOnDate =
                            dt.getTime() ==
                            rec.data[Ext.ensible.cal.EventMappings.StartDate.name].clearTime(true).getTime();
                        var spansFromPrevView =
                            w == 0 && d == 0 && dt > rec.data[Ext.ensible.cal.EventMappings.StartDate.name];
            ...
            if (Ext.ensible.Date.diffDays(evt.data[M.StartDate.name], evt.data[M.EndDate.name]) > 0) {
                var daysInView =
                    Ext.ensible.Date.diffDays(
                        ...
                        Ext.ensible.Date.min(me.viewEnd, evt.data[M.EndDate.name]),
                    ) + 1;
   ...
   callback: fn  // required to refresh the view after the fx finish
    ...
    isOverlapping: function (evt1, evt2) {
        ...
        var evtsOverlap = Ext.ensible.Date.rangesOverlap(start1, end1, start2, end2),
            ...
            ev1MinHeightOverlapsEv2 =
                minimumMinutes > 0 && startDiff > -minimumMinutes && startDiff < minimumMinutes;
    ...
    setViewBounds: function (startDate) {
        ...
        switch (this.weekCount) {
            ...
            case 1:
                this.viewStart =
                    this.dayCount < 7 && !this.startDayIsStatic
                        ? start
                        : start.add(Date.DAY, -offset).clearTime(true);
   ...
   */
    ...
    getEventRecord: function (id) {
        var idx = this.store.find(
            Ext.ensible.cal.EventMappings.EventId.name,
            id,
            0, // start index
    ...
    moveEvent: function (rec, dt) {
        if (
            Ext.ensible.Date.compare(rec.data[Ext.ensible.cal.EventMappings.StartDate.name], dt) === 0
        ) {
            // no changes
            return;
        }
        if (this.fireEvent('beforeeventmove', this, rec, dt) !== false) {
            ...
            rec.set(
                ...
                rec.data[Ext.ensible.cal.EventMappings.EndDate.name].add(Date.MILLI, diff),
            );
            rec.endEdit();
            this.save();
...
Ext.ensible.cal.MonthView = Ext.extend(Ext.ensible.cal.CalendarView, {
    ...
    getTemplateEventData: function (evt) {
        ...
        data.Title =
            (evt[M.IsAllDay.name] ? '' : evt[M.StartDate.name].format(fmt)) +
    ...
    getViewPadding: function (sides) {
        var sides = sides || 'tlbr',
            ...
            height =
                this.showHeader && top ? this.el.select('.ext-cal-hd-days-tbl').first().getHeight() : 0,
            width = 0;
...
Ext.ensible.cal.DayBodyView = Ext.extend(Ext.ensible.cal.CalendarView, {
    ...
    onEventResize: function (rec, data) {
        if (this.fireEvent('beforeeventresize', this, rec, data) !== false) {
            ...
            if (
                D.compare(rec.data[start], data.StartDate) === 0 &&
    ...
    getTemplateEventBox: function (evt) {
        var heightFactor = this.hourHeight / this.hourIncrement,
            ...
            endOffset = Math.min(
                end.getHours() - this.viewStartHour,
        ...
        evt._height =
            Math.max((endMins - startMins) * heightFactor, this.minEventHeight) + evtOffsets.height;
    },
            ...
            for (j = 0; j < l; j++) {
                ...
                if (this.isOverlapping(evt, evt2)) {
                    ...
                    if (i < j) {
                        ...
                        overlapCols[dt] = overlapCols[dt]
                            ? Math.max(overlapCols[dt], evt2._overcol)
                            : evt2._overcol;
                    }
                }
            }
    ...
    getDayAt: function (x, y) {
        var sel = '.ext-cal-body-ct',
            ...
            dt = this.viewStart
                .add(Date.DAY, dayIndex)
                .add(Date.MINUTE, mins)
                .add(Date.HOUR, this.viewStartHour),
            el = this.getDayEl(dt),
            timeX = x;
...
Ext.ensible.cal.CalendarPanel = Ext.extend(Ext.Panel, {
   ...
   */
    ...
    initComponent: function () {
        ...
        this.activeItem =
            this.activeItem === undefined ? idx : this.activeItem > idx ? idx : this.activeItem;
